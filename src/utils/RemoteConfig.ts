...
export class RemoteConfig {
  ...
  async sync() {
    if (
      ...
      const urlBase = this.configuration.url_base
      const apiBase = this.configuration.api_base
      if (urlBase && apiBase) {
        const departementsListPath =
          this.configuration!.path_list_departments ||
          `/vitemadose/departements.json`
        const infosDepartementPath =
          this.configuration!.path_data_department || `/vitemadose/{code}.json`
        this._urlGenerator = {
          listDepartements: () => `${urlBase}${departementsListPath}`,
          infosDepartement: codeDepartement =>
            `${urlBase}${infosDepartementPath.replace('{code}', codeDepartement)}`,
          workshops: () => `${apiBase}`,
        }
      }
